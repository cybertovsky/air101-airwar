...
lcd.init("st7735",{port = "device",pin_dc = pin.PE08 ,pin_rst = pin.PC12,pin_pwr = pin.PE09,direction = 2,w = width,h = height,xoffset = 24,yoffset = 0},spi_lcd)
...
sys.taskInit(function()
    ...
    gpio.setup(pin.PE06, function(val)
        -- up
        if val == 0 then
            player.vy = -1
            player.vx = 0
    ...
    gpio.setup(pin.PA10, function(val)
        -- down
        if val == 0 then
            player.vy = 1
            player.vx = 0
    ...
    gpio.setup(pin.PB04, function(val)
        -- left
        if val == 0 then
            player.vx = -1
            player.vy = 0
    ...
    gpio.setup(pin.PC07, function(val)
        -- right
        if val == 0 then
            player.vx = 1
            player.vy = 0
...
sys.timerLoopStart(function()
    ...
    --lcd.clear(0x0000)
    lcd.fill(0, 0, 80,20, 0x0000)
    lcd.drawStr(2, 10, "score:" .. score, 0xFFFF)
    lcd.fill(player.x, player.y, player.x + player.width, player.y + player.height, 0x0000)
    ...
    for i, v in ipairs(projectiles) do
        lcd.fill(v.x, v.y, v.x + v.width, v.y + v.height, 0x0000)
        v.y = v.y - v.speed
    ...
    for i, a in ipairs(projectiles) do
        for ii, b in ipairs(enemys) do
            if checkCollision(a, b, 0, 0, 0, 0) then
                ...
                lcd.fill(a.x, a.y, a.x + a.width, a.y + a.height, 0x0000)
                lcd.fill(b.x, b.y, b.x + b.width, b.y + b.height, 0x0000)
                score = score + 1
            end
        end
    end
